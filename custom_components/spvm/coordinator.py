...
class SPVMCoordinator(DataUpdateCoordinator[SPVMData]):
    """Compute expected solar production (very first minimal version)."""
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry) -> None:
        ...
        # Required
        self.pv_entity: str = data.get(CONF_PV_SENSOR)
        self.house_entity: str = data.get(CONF_HOUSE_SENSOR)
    ...
    async def _async_update_data(self) -> SPVMData:
        ...
        if self.unit_power == UNIT_KW:
            pv_w = pv * KW_TO_W
        else:
            pv_w = pv

        # ---- Minimal internal expected model ---------------------------------
        # Start from "clear-sky expectation" ≈ observed PV clamped to physical caps,
        # then apply corrections (degradation, cloud cover). This is deliberately
        # simple for v0.6.0 minimal footprint; improved model can replace it later.

        expected_clear_w = float(min(max(pv_w, 0.0), float(self.cap_max_w)))

        # Degradation (simple linear factor)
        deg_factor = max(0.0, 1.0 - float(self.degradation_pct) / 100.0)
        expected_after_deg = expected_clear_w * deg_factor

        # Cloud cover correction (0..100)
        ...
        # Reserve Zendure (ne pas “promettre” la réserve comme disponible)
        ...
        # Expose helpful live inputs when available
        if grid is not None:
            attrs["grid_now"] = grid
        if batt is not None:
