...
class SPVMConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            # Normalisation/validation (inclut d̩faut expected_sensor + bool debug)
        ...
        # Build schema (avec d̩fauts visibles)
    ...
    def _coerce_bool(value: Any, default: bool = False) -> bool:
        """Convertit proprement vers bool (g̬re str/bool/entit̩ h̩rit̩e)."""
        ...
        if isinstance(value, str):
            ...
            if s in ("0", "false", "off", "no", "non", ""):
                return False
            # ancien bug: une entit̩ 'sensor.xxx' stock̩e par erreur -> consid̩rer False
        ...
        return vol.Schema(
            {
                ...
                # Debug (bool̩en, plus une entit̩)
                vol.Optional(
                    CONF_DEBUG_EXPECTED,
...
class SPVMOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        """Manage the options."""
        if user_input is not None:
            # Normalisation (inclut d̩faut expected_sensor + bool debug)
